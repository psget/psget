...
Describe "Install-Module" {
    ...
    Context "When modules from centralized PsGet repository" {
        ...
        #install-module Foo -DirectoryURL "https://github.com/psget/psget/raw/master/TestModules/Directory.xml" -Verbose:$verbose
    ...
    Context "Side effects" {
        ...
        It "Should modify User permanent environment variable When using PersistEnvironment switch" {
            #Use a unique path so it can be removed from the environment variable
            $tempDir = Get-TempDir
            $beforeModulePath = $env:PSModulePath
            try{
                ...
                Remove-PathFromEnvironmentVariable -VariableName "PSModulePath" -PathToRemove $tempDir -PersistEnvironment
            }
    ...
    Context "When requireing a specific module hash value" {
    ...
    Context "When installing Nuget modules" { 
